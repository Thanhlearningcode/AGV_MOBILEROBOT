...
const double TICKS_PER_REVOLUTION = 2688; // For reference purposes.
const double WHEEL_RADIUS = 0.085; // Wheel radius in meters
const double WHEEL_BASE = 0.36; // Center of left tire to center of right tire
...
int main(int argc, char **argv) {
  ...
  while(ros::ok()) {
    ...
    if(initialPoseRecieved) {
      ...
      odom_trans.transform.translation.z = quatOdom.pose.pose.position.z;
      odom_trans.transform.rotation.x = quatOdom.pose.pose.orientation.x;
      odom_trans.transform.rotation.y = quatOdom.pose.pose.orientation.y;
      odom_trans.transform.rotation.z = quatOdom.pose.pose.orientation.z;
      odom_trans.transform.rotation.w = quatOdom.pose.pose.orientation.w;
      //send the transform
