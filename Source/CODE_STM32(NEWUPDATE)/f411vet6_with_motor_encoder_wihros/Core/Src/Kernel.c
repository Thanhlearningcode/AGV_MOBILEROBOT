...
void osKernelLaunch ( uint32_t quanta ){
	...
	//os launch scheduler
	// Start the cooperative scheduler by setting the SysTick to fire at 'quanta' ms
	// and jump into the first thread when SysTick interrupt occurs.
	osSchedulerLaunch();
}

// Simple cooperative scheduler launcher: set currentPt and return to thread context
void osSchedulerLaunch(void){
	// Point currentPt at the first thread, then pend a context switch
	currentPt = &tcbs[0];
	// Request PendSV to perform the first context switch
	SCB->ICSR = SCB_ICSR_PENDSVSET_Msk;
}

// SysTick handler helper used by the kernel
void SysTick_Handler1(){
	// On each SysTick, request PendSV for a context switch (preemptive)
	SCB->ICSR = SCB_ICSR_PENDSVSET_Msk;
}

// Cooperative yield API: caller yields execution to next thread
void osThreadYield(void){
	// mark PendSV to perform a context switch
	SCB->ICSR = SCB_ICSR_PENDSVSET_Msk;
}

// Weak default idle task: low priority no-op loop
...
// PendSV handler: save/restore callee-saved registers and switch PSP
// PendSV handler is implemented in the vector table file (stm32f4xx_it.c)
