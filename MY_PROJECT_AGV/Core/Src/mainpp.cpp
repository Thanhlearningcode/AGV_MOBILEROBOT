...
//		rightVelPub.publish( &right_vel_data );
	...
	if (is_recv_left_wheel == 1 && is_recv_right_wheel == 1){
		...
		float vtl = abs(vel_left);	  // Lấy vận tốc mong muốn (tuyệt đối).
		float kpl = 5;	// Hệ số khuếch đại Proportional (P).
		float kil = 0;	// Hệ số khuếch đại Integral (I).
		float el = vtl - abs(left_wheel_vel);	 // Sai số giữa vận tốc mong muốn và thực tế.
		eintegral_l = eintegral_l + el*deltaT;	// Cộng dồn sai số theo thời gian (thành phần I).

		float ul = kpl*el + kil*eintegral_l;	// Tính giá trị điều khiển tổng hợp P và I.

		pwmLeftReq = fabs(ul);	// PWM yêu cầu (tuyệt đối).

		if(pwmLeftReq > PWM_MAX){
			pwmLeftReq = PWM_MAX;	  // Giới hạn giá trị PWM tối đa.
		}

		// -- set vel target for right motor --
		float vtr = abs(vel_right); 	 // Lấy vận tốc mong muốn.
		float kpr = 5;
		float kir = 0;
		float er = vtr - abs(right_wheel_vel);	// Sai số của bánh phải.
		eintegral_r = eintegral_r + er*deltaT;	// Cộng dồn sai số.

		float ur = kpr*er + kir*eintegral_r;	// Điều khiển tổng hợp P và I.

		pwmRightReq = fabs(ur);

		if(pwmRightReq > PWM_MAX){
			pwmRightReq = PWM_MAX;	     // Giới hạn PWM.
		}
